    ...
    def loss_omega_mat(self,mat_d):
        ...
        for aux_mi in range(mat_omega_all.shape[0]):
            for aux_ni in range(mat_omega_all.shape[1]):
                ...
                '''TODO: (test)'''
                test_counter += 1
                if(test_counter % ( int( (mat_d.shape[0]*mat_d.shape[1]) / 20 ) ) == 0):
...
if __name__ == '__main__':
    ...
    graphlossdn = GraphLossDN(pLambda=0.1, pAlpha=3.5, pCigma_int=0.07, pCigma_spa=3, pB=7, pK=10, pQ=3)      
    ...
    cv_MatI = cv.resize(cv_MatI, (int(0.07*orig_rows),int(0.07*orig_cols)))
